...
object TimeUsage {
  ...
  def row(line: List[String]): Row =
    Row.fromSeq(line.head :: line.tail.map(col => col.toDouble))
  ...
  def classifiedColumns(columnNames: List[String]): (List[Column], List[Column], List[Column]) = {
    val primary = columnNames.
      filter(c => List("t01", "t03", "t11", "t1801", "t1803").exists(e => c.startsWith(e))).
      map(c => col(c))

    val working = columnNames.
      filter(c => List("t05", "t1805").exists(e => c.startsWith(e))).
      map(c => col(c))

    val other = columnNames.
      filter(c =>
        List("t02", "t04", "t06", "t07", "t08", "t09", "t10", "t12", "t13", "t14", "t15", "t16", "t18").exists(e => c.startsWith(e)) &&
        !List("t1801", "t1803", "t1805").exists(e => c.startsWith(e))
      ).
      map(c => col(c))
