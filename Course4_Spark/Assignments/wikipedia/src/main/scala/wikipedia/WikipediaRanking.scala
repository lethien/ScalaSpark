...
object WikipediaRanking {
  ...
  def occurrencesOfLang(lang: String, rdd: RDD[WikipediaArticle]): Int = rdd.
    filter(article => article.text.split(" ").contains(lang)).
    count().intValue()
  ...
  def makeIndex(langs: List[String], rdd: RDD[WikipediaArticle]): RDD[(String, Iterable[WikipediaArticle])] = rdd.
    flatMap(article => for (lang <- langs if article.text.split(" ").contains(lang)) yield (lang, article)).
    groupByKey()
  ...
  def rankLangsReduceByKey(langs: List[String], rdd: RDD[WikipediaArticle]): List[(String, Int)] = rdd.
    flatMap(article => for (lang <- langs if article.text.split(" ").contains(lang)) yield (lang, 1)).
    reduceByKey(_ + _).
    sortBy(_._2, false).
    collect().toList
