...
trait Solver extends GameDef {
  ...
  def neighborsWithHistory(b: Block, history: List[Move]): Stream[(Block, List[Move])] = {
    b.legalNeighbors.toStream.map(neighbor => (neighbor._1, neighbor._2 :: history))
  }
  ...
  def from(initial: Stream[(Block, List[Move])],
    ...
    if(initial.isEmpty) Stream.empty
      ...
      val more = for {
        ...
        nextMove <- newNeighborsOnly(neighborsWithHistory(block, history), explored)
      } yield nextMove
      initial #::: from(more, explored ++ (more map (_._1))) // concat stream
    }
  }
